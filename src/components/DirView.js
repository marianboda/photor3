...
export const DirView = ({ data }) => {
    ...
    const numberRange = (firstNumber && lastNumber)
    ...
    return (
        <div style={{ padding: 2 }}>
            ...
            {isRawAndJpg && <Label color="violet" horizontal>RAW + JPG</Label>}
            {
                numberRange &&
                    <Label color={getFractionColor(fraction)} horizontal>{Math.round(fraction*10)/10}</Label>
            }
        </div>
    );
}
